...
class Move {
    ...
    private Move(int col0, int row0, int col1, int row1) {
        ...
        int indDiff = abs(_fromIndex - _toIndex);
        if (indDiff == 1 || indDiff == 10 || indDiff == 11 || indDiff == 12) {
            _isJump = false;
            _isExtend = true;
        } else {
            _isExtend = false;
    ...
    public String toString() {
        if (isPass()) {
            ...
            String str = "";
            str += Character.toString(col0());
            str += Character.toString(row0());
            str += "-";
            str += Character.toString(col1());
            str += Character.toString(row1());
            return str;
        }
    }
